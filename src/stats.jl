...
## diagonal covariance
function stats(gmm::GMM{GT,DCT}, x::Matrix{T}, order::Int; weights::Union{Vector{T},Nothing}=nothing) where DCT<:DiagCov{GT} where {GT,T}
...
function stats(gmm::GMM{GT,FCT}, x::Array{T,2}, order::Int; weights::Union{Vector{T},Nothing}=nothing) where FCT<:FullCov{GT} where {GT,T}
    RT = promote_type(GT, T)
    (nₓ, d) = size(x)
    ng = gmm.n
    gmm.d == d || error("dimension mismatch for data")
    ...
    log_lpf = logsumexp(ll .+ log.(gmm.w)', 2)
    if weights !== nothing
        llh = sum(log_lpf .* weights)
        for j = 1:ng
            for i = 1:nₓ
                @inbounds γ[i, j] *= weights[i]
            end
...
function stats(gmm::GMM, x::Matrix{T}; order::Int=2, parallel=false, weights::Union{Vector{T},Nothing}=nothing) where T<:AbstractFloat
    parallel &= nworkers() > 1
    ng = gmm.n
    (nₓ, d) = size(x)
    if weights !== nothing
    ...
    if weights !== nothing
        ww = Vector{T}[weights[round(Int, i * l + 1):round(Int, (i + 1)l)] for i = 0:(blocks-1)]
    end

    if parallel
        if weights !== nothing
            r = pmap((x, w) -> stats(gmm, x, order; weights=w), xx, ww)
        else
        ...
        if weights !== nothing
            r = stats(gmm, popfirst!(xx), order; weights=popfirst!(ww))
            for (x, w) in zip(xx, ww)
                r += stats(gmm, x, order; weights=w)
            end
        else
